...
import hello.proxy.config.v2_dynamicproxy.LogTraceFilterHandler;
...
public class DynamicProxyFilterConfig {
	...
	public OrderControllerV1 orderControllerV1(LogTrace logTrace) {
		...
		OrderControllerV1 proxy = (OrderControllerV1) Proxy.newProxyInstance(
			...
			new LogTraceFilterHandler(orderController, logTrace, PATTENRS) // JDK 기반 프록시 생성(InvocationHandler를 구현하는 클래스)
		);
	...
	public OrderServiceV1 orderServiceV1(LogTrace logTrace) {
		...
		OrderServiceV1 proxy = (OrderServiceV1)Proxy.newProxyInstance(
			...
			new LogTraceFilterHandler(orderService, logTrace, PATTENRS) // JDK 기반 프록시 생성(InvocationHandler를 구현하는 클래스)
		);
	...
	public OrderRepositoryV1 orderRepositoryV1(LogTrace logTrace) {
		...
		OrderRepositoryV1 proxy = (OrderRepositoryV1) Proxy.newProxyInstance(
			...
			new LogTraceFilterHandler(orderRepository, logTrace, PATTENRS) // JDK 기반 프록시 생성(InvocationHandler를 구현하는 클래스)
		);
